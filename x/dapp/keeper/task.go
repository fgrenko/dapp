...
func (k Keeper) AppendTask(ctx sdk.Context, task types.Task) uint64 {
  ...
  task.Id = count
  // Get the store
  store := prefix.NewStore(ctx.KVStore(k.storeKey), []byte(types.TaskKey))
  // Convert the post ID into bytes
  byteKey := make([]byte, 8)
  binary.BigEndian.PutUint64(byteKey, task.Id)
...
func (k Keeper) GetTaskCount(ctx sdk.Context) uint64 {
	...
	store := prefix.NewStore(ctx.KVStore(k.storeKey), []byte(types.TaskCountKey))
	// Convert the PostCountKey to bytes
	byteKey := []byte(types.TaskCountKey)
